...
class Principal(Screen):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            ...
            self.rect = Rectangle(size = (self.size[0], 15 * self.size[1]), pos = (0, 0.85 * self.height))
        ...
        self.add_widget(Button(text = "Borrar", bold = True, size_hint = (0.25, 0.12), background_color = "red", on_press = lambda x: self.borrar(), pos_hint = {"center_x": 0.5}))

    def on_size(self, *args):

        self.rect.size  = (self.size[0], 15 * self.size[1])
